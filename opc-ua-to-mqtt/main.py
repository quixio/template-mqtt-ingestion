...
def main():
    ...
    app = Application(
        consumer_group="opc-ua-to-mqtt",
        auto_create_topics=True,
        auto_offset_reset="earliest"
    )
    input_topic = app.topic(name=os.environ["input"], key_deserializer="str")
    output_topic = app.topic(name=os.environ["output"], value_serializer="double")
    sdf = app.dataframe(topic=input_topic)

    sdf["root"] = sdf.apply(lambda row, key, *_: key, metadata=True)

    # Do StreamingDataFrame operations/transformations here
    sdf = sdf.group_by(lambda row: row["root"] + "/" + row["param"], "group-by-machine")
    sdf = sdf.apply(lambda row: float(row["val"]))
